...
BOOL Base64EncodeA(void** dest, unsigned long *dlen, void* src, unsigned long slen) {
	...
	if(!CryptBinaryToStringA_p(src, slen, CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF, NULL, dlen)){
		BeaconPrintf(CALLBACK_OUTPUT, "[-] Failed to initialize encoding.\n");
		return FALSE;
	}
	
	if(!CryptBinaryToStringA_p(src, slen, CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF, *dest, dlen)){
		BeaconPrintf(CALLBACK_OUTPUT, "[-] Failed to encode the memory block.\n");
		return FALSE;
	}
...
int isEDRString(const char* str) {
	...
	char* fullHash = "0x";
	BeaconPrintf(CALLBACK_OUTPUT, "[*] Received string: %s | %s | Hashed out: ",str,lowerStr);
	
	// Why tf it is printing out random shit
	BeaconPrintf(CALLBACK_OUTPUT, "0x");
	for (int i = 3; i >= 0; i--) {
		char tmp[2];
		BeaconPrintf(CALLBACK_OUTPUT, "%02X", hashedStr[i]);
		MSVCRT$sprintf(tmp, "%02X", hashedStr[i]);
		MSVCRT$strcat(fullHash, tmp);
	}
...
void checkServices() {
	char* collectedStr = "";
	BeaconPrintf(CALLBACK_OUTPUT, "Checking services where EDR/AV artifacts can be found...\n");
    ...
    if (!hSCManager) {
        BeaconPrintf(CALLBACK_OUTPUT, "[-] Failed to open Service Control Manager\n");
        return;
    }
	...
	*/
	...
	ENUM_SERVICE_STATUS_PROCESSA* services = (ENUM_SERVICE_STATUS_PROCESSA*)MSVCRT$malloc(bytesNeeded); 	// The number of bytes allocated depends on the number of service entries extracted.
    if (services == NULL) {
        BeaconPrintf(CALLBACK_OUTPUT, "[-] Failed to allocate memory for services\n");
    ...
    if (!pCloseServiceHandle(hSCManager, 
							...
							NULL)) {
        BeaconPrintf(CALLBACK_OUTPUT, "[-] Failed to enumerate services\n");
	...
	*/
            ...
            if (pServiceConfig != NULL) {
                if (pQueryServiceConfigA(hService, pServiceConfig, bytesNeeded, &bytesNeeded)) { // Returns the service configuration in the buffer (2nd param)
                    ...
                    if (isEDRString(services[i].lpServiceName)) { // Checks if the "serviceInfo" (service-name to be exact) matches atleast one EDR in the list of security solutions.
                        MSVCRT$strcat(collectedStr, "[!] Suspicious service found:\n");
                        char* strTmp1;
						MSVCRT$sprintf(strTmp1, "\tName: %s\n", services[i].lpServiceName);
						MSVCRT$strcat(collectedStr, strTmp1);
						
						char* strTmp2;
                        MSVCRT$sprintf(strTmp2, "\tDisplay Name: %s\n", services[i].lpDisplayName);
						MSVCRT$strcat(collectedStr, strTmp2);
						
						char* strTmp3;
                        MSVCRT$sprintf(strTmp3, "\tBinary Path: %s\n\n", pServiceConfig->lpBinaryPathName);
						MSVCRT$strcat(collectedStr, strTmp3);
                        foundSuspicious = 1;
                    }
                }
	...
	pCloseServiceHandle(hSCManager);
    if (!foundSuspicious)
        BeaconPrintf(CALLBACK_OUTPUT, "[+] No suspicious services found\n\n");
	return;
}
...
void go(char *args, int alen) {
	BeaconPrintf(CALLBACK_OUTPUT, "[+] Listing processes..\n");
