...
*/
#include "eva-eppblk-fw-bof-imports.h"
#include "eva-eppblk-fw-bof-structs.h"
...
void go(char *args, int alen) {
    ...
    if (FAILED(hrComInit)) {
        PRINT("[*] CoInitializeEx failed: 0x%08lx\n", hrComInit);
        goto Cleanup;        
    }
    ...
    if (FAILED(hr)) {
        PRINT("[*] CoCreateInstance for INetFwPolicy2 failed: 0x%08lx\n", hr);
        goto Cleanup;        
    }
    ...
    if (FAILED(hr)) {
        PRINT("[-] get_Rules failed: 0x%08lx\n", hr);
        goto Cleanup;
    }
    ...
    if (FAILED(hr)) {
        PRINT("[-] CoCreateInstance for Firewall Rule failed: 0x%08lx\n", hr);
        goto Cleanup;
    }
    ...
    */
    ...
    if (FAILED(hr)) {
        PRINT("[-] Firewall Rule Add failed: 0x%08lx\n", hr);
        goto Cleanup;
    }
